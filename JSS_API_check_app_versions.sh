...
jamfAppsXml=$(curl -s -H "Accept: application/xml" -H "Content-type: application/xml" -u $apiReadOnlyUser:"$apiReadOnlyPass" $JSSapiPath | xpath '//mobile_device_applications/mobile_device_application' 2>&1 | awk -F'<mobile_device_application>|</mobile_device_application>' '{print $2}' | grep .)

function write_app_csv () {

#write csv header
echo "status,jss_id,bundleid,itunes_lastknown_url,jamf_version,apple_version" > ${csvFile}
...
while read xml_string; do
	...
	itunes_lastknown_url_raw=$(curl -s -u $apiReadOnlyUser:"$apiReadOnlyPass" $JSSapiPath/id/$id | xpath '//mobile_device_application/general/itunes_store_url' 2>&1 | awk -F'<itunes_store_url>|</itunes_store_url>' '{print $2}' | grep .)
	...
	appleVersion=$(/usr/local/bin/jq -r ".results.\"${itunesAdamId}\".offers[].version.display" <<< "${itunes_data}" 2>/dev/null)
  	...
  	if [[ -z $id ]]; then
  		...
  		echo "NO LONGER AVAILABLE,$id,$app_bundle_id,$itunes_lastknown_url,$jamf_version,$appleVersion" >> ${csvFile}
  	elif [[ ${jamf_version} != ${appleVersion} ]]; then
  		echo "VERSION MISMATCH,$id,$app_bundle_id,$itunes_lastknown_url,$jamf_version,$appleVersion" >> ${csvFile}
	elif [[ -n ${id} ]]; then
		echo "CURRENT,$id,$app_bundle_id,$itunes_lastknown_url,$jamf_version,$appleVersion" >> ${csvFile}
  	fi
